...
- **8/18/26 — v0.8.0: CocoIndex Integration (optional)**: Rust-backed [CocoIndex](https://github.com/cocoindex-io/cocoindex) ingest pipeline (`PIPELINE_BACKEND=cocoindex`) that mixes CocoIndex connectors with Flexible GraphRAG sources (including event detectors), parsers, chunkers, embeddings, and LlamaIndex/LangChain KG extractors, writing to all 15 property graph, 10 vector, 4 RDF and 3 search stores. Adds step-level memoization and automatic delete reconciliation; same UI / REST / MCP. Also runs standalone via `cocoindex update app.py`. Mutually exclusive with `ENABLE_INCREMENTAL_UPDATES=true` and `ENABLE_LANGFLOW_FLOWS=true`. Now with **custom KG extractors** (`KG_EXTRACTOR_BACKEND` accepts your own `KGExtractor`, which can delegate unrecognised documents to the built-in one) and **entity resolution** (`ENTITY_RESOLUTION=normalize|llm`). New **meeting-notes example** ([`examples/cocoindex/meeting_notes_graph_any/`](https://github.com/stevereiner/flexible-graphrag/tree/main/examples/cocoindex/meeting_notes_graph_any)) — a CocoIndex example ported to run against any configured graph store and source. See [CocoIndex Integration](GETTING-STARTED/COCOINDEX-INTEGRATION.md).
- **8/8/26 — v0.7.2: Nuxeo data source, Alfresco OAuth2/ticket, ACS 26.1, MCP OAuth**: **Nuxeo** added as a data source across all 3 UIs plus REST/MCP, with basic / token / OAuth2 auth and real-time incremental sync via the Nuxeo audit event stream (Kafka). **Alfresco OAuth2 and ticket** authentication added for initial ingest and real-time sync. **Alfresco Community 26.1** Docker upgrade. The **MCP server** gained optional OAuth2 on its transport and moved to **FastMCP 3**.
- **7/20/26 — v0.7.1: LiteParse, Langflow Docker, MS Graph delta**: Document processing supports **LiteParse** alongside **Docling** and **LlamaParse**. Langflow integration ships with fixes and an optional **Langflow Docker** image bundling the 12 "Flexible" components. **MS Graph delta query** support added for more efficient incremental updates from SharePoint and OneDrive.
- **7/5/26 — Langflow Visual Flows (optional)**: The ingest pipeline, hybrid search, and AI query can run through customizable [Langflow](https://www.langflow.org/) flows (12 custom Flexible GraphRAG components), using your existing `.env` config. See [Langflow Integration](GETTING-STARTED/LANGFLOW-INTEGRATION.md).
- **5/6/26 — 15 Property Graph Databases**: 8 supported on both LlamaIndex and LangChain, 1 LI-only (Google Cloud Spanner Graph), 6 LC-only (ArangoDB, Apache AGE, Azure Cosmos DB for Gremlin, Apache HugeGraph, SurrealDB, TigerGraph). AWS Neptune RDF/SPARQL added.
- **5/5/26 — Full LangChain Support**: Added full peer-framework support for LangChain alongside LlamaIndex. All 8 original property graph databases (Neo4j, ArcadeDB, FalkorDB, Ladybug, Memgraph, NebulaGraph, Amazon Neptune, Neptune Analytics) now work with both frameworks. 7 new LangChain-only property graph databases added: ArangoDB, Apache AGE, Azure Cosmos DB for Gremlin, Apache HugeGraph, SurrealDB, TigerGraph, and Google Cloud Spanner. All 10 vector databases, all 3 search engines, and all LLM and embedding providers work with both LlamaIndex and LangChain. Every stage of the ingestion pipeline and retrieval fusion can be configured independently — chunking, KG extraction, graph write, vector write, search write, and retrieval fusion. Note: data source reading (first stage) is LlamaIndex only; RDF graph database store adapters are framework-independent with LangChain Text-to-SPARQL retrieval.
