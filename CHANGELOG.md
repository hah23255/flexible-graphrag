...
- **Nuxeo added as a data source** — ingest from a Nuxeo repository through all three UIs (React/Vue/Angular) and via the REST API / MCP server (`nuxeo_config`). Supports `basic`, `token` (X-Authentication-Token), and `oauth2` (Bearer) auth, path/node selection, and real-time incremental sync via the Nuxeo audit event stream (Kafka).
- **Alfresco OAuth2 and ticket authentication (data source)** — the Alfresco source now supports `basic`, `ticket`, and `oauth2` (OIDC Bearer validated by Alfresco's built-in `identity-service`, e.g. against Keycloak); all three UIs (React/Vue/Angular) gained an auth-method selector with the OAuth2/ticket fields. Works for initial ingest **and** real-time incremental sync (add/update/delete). Requires `python-alfresco-api >= 1.2.1`.
- **Nuxeo token auth self-fetches from username/password** (`sources/nuxeo.py`) — token mode now works like Alfresco ticket auth: give username/password and the source fetches a reusable Nuxeo auth token (`GET /nuxeo/authentication/token`) and uses it as `X-Authentication-Token`. The API still accepts a pre-obtained `token` directly. All 3 UIs (React/Vue/Angular) show username/password for token mode instead of a token field.
- **Optional MCP transport authentication** (`flexible-graphrag-mcp/main.py`, `MCP_TRANSPORT_AUTH=true`) — secures the MCP server itself: HTTP callers must present an OAuth2 bearer token, validated against an OIDC IdP's JWKS (FastMCP JWT verifier). Off by default; stdio unaffected. Env: `MCP_AUTH_JWKS_URI` / `MCP_AUTH_ISSUER` / `MCP_AUTH_AUDIENCE`.
- **MCP config arguments accept an object or a JSON string** (`flexible-graphrag-mcp/main.py`) — the `*_config` params (and `paths`) now take either structured JSON (from clients like MCP Inspector) or a stringified blob (from LLM clients), so calling the tools from an MCP Inspector no longer fails validation.
- **OAuth2 token-minting helper scripts** (`scripts/alfresco/`, `scripts/nuxeo/`, `scripts/mcp/`) — small local-dev helpers to obtain the tokens you paste into the UIs / MCP config: an Alfresco **user** token (Keycloak password grant), a Nuxeo user token (auth-code + PKCE, interactive or headless auto-grant), and an **MCP transport** bearer (client_credentials). PowerShell / `.bat` (built-in `curl.exe`) / `.sh` / Python variants, each with a README; default to the bundled local Keycloak realm.
- **Nuxeo + Kafka deployment reference** (`examples/nuxeo-deployment/`) — a copy of the `compose.yaml` + `kafka.conf` that enables Nuxeo's audit stream over Kafka for real-time sync, built on Angel Borroy's content-lake POC deployment (unofficial), with deployment-agnostic setup steps documented in `docs/DATA-SOURCES/README-nuxeo.md`.
- **Data-source documentation** — a dedicated **`docs/DATA-SOURCES/README-alfresco.md`** (the three Alfresco auth methods, the compose-only Keycloak / `identity-service` OAuth2 setup, client-credentials-vs-user-token, token-minting scripts, real-time sync — parallel to `README-nuxeo.md`); a **Nuxeo** section + screenshot in the UI guide (`docs/UI-GUIDE/UI-TAB-SOURCES.md`) and an Authentication line on the Alfresco section; and the data-source **counts / enumerations updated across the docs** (13 → 14 sources, 9 → 10 auto-sync, Nuxeo added to the source lists; refreshed `index.md`'s graph/RDF/parser counts).

### Changed

- **Alfresco Docker stack upgraded to Community 26.1.0** (`docker/includes/alfresco.yaml`) — repo/share 26.1.0, search-services 2.0.18, ActiveMQ 6.2.1, transform 5.4.0, content-app 7.3.0, control-center 10.3.0, Postgres 17.9 (still Solr; matches acs-deployment `v10.3.0`). Added the ActiveMQ admin credentials that 6.x now requires. Postgres 17 needs a fresh Alfresco DB volume (major-version bump).
- **flexible-graphrag-mcp on FastMCP 3** — pinned `fastmcp>=3.4.5,<4`; JWT transport auth now uses `JWTVerifier` directly (FastMCP 3 removed the v2 `BearerAuthProvider`).
- **`python-alfresco-api` dependency bumped to `>= 1.2.1`** (from 1.1.5) — 1.2.1 provides the OAuth2/ticket auth utilities used above and tolerates OAuth2 service-account responses that omit `UserInfo.displayName` (JIT users), so an OAuth2 client-credentials ingest doesn't error on `createdByUser`/`modifiedByUser`.
- **Packaging: hatchling build backend** — `flexible-graphrag` and `flexible-graphrag-mcp` switched their build backend from setuptools to hatchling (quieter builds; consistent with `python-alfresco-api`). **Caveat:** hatchling's editable install can't shadow a same-named installed package, so this repo's bundled `langchain` package no longer auto-wins `import langchain` in an editable venv the way setuptools' editable finder did. The **host Langflow** launcher (a console-script) now needs `PYTHONPATH=<backend dir>` so our `langchain.graph/search` resolve (otherwise Search / AI Query build with `No module named 'langchain.graph'`); `python main.py` is unaffected (script dir already on `sys.path`) and Docker already sets `PYTHONPATH=/app`. Documented in `docs/GETTING-STARTED/LANGFLOW-INTEGRATION.md`.
- **Regenerated the Langflow flows** (`flows/fg_*.json`) — app-driven flow mode runs each flow's embedded component code, so the flows are rebuilt (via `langflow_components/generate_flows.py`) whenever a component or the package version changes. All 4 flows now embed flexible-graphrag `0.7.2` and langflow `1.11.2`, and the ingestion flow's `FlexibleDataSource` includes the `nuxeo` data source. (`generate_flows.py` now stamps the flexible-graphrag version into each component's dependency metadata — langflow's own detection reports it as `null` under an editable install, which writes no `top_level.txt`.)
- **Langflow updated to 1.11.2** (host quickstart in `LANGFLOW-INTEGRATION.md` + `Dockerfile.langflow`) — 1.11.1/1.11.2 fix bugs over 1.11.0 and install cleanly with fastapi 0.141.1 (no pin needed; a `fastapi==0.139.2` pre-pin remains documented only as a fallback for the old `get_body_field` break). Both install paths restore PyJWT after the flexible-graphrag install, since `nuxeo[oauth2]` pulls a `jwt` package that shadows the PyJWT Langflow needs. **Nuxeo in flow mode:** `sources/nuxeo.py` applies a compat shim (grafts `JWT`/`jwk_from_dict`/`JWTDecodeError` onto PyJWT) so **all 3 Nuxeo auths (basic/token/oauth2) work in flow mode** — OAuth2 uses a pre-obtained Bearer (minted by `scripts/nuxeo`), so ingest never decodes a JWT. Alfresco + all other sources work fully in flow mode. Also switched the documented `uv` install flag from the deprecated `--native-tls` to `--system-certs`.

### Fixed

- **Real-time Alfresco event sync broke on ActiveMQ 6.x** (`incremental_updates/detectors/alfresco_broadcaster.py`) — the STOMP consumer passed `password=` to stomp.py, whose kwarg is `passcode=`, so the passcode was empty. Harmless on ActiveMQ 5.x (no broker auth) but ACS 26.1's ActiveMQ 6.x enforces auth, giving `User name [admin] or password is invalid`. Fixed to `passcode=`.
- **OAuth2 client-credentials ingest failed from the UI** (`sources/alfresco.py`) — the UI omits `grant_type`, so it arrived as `null`; `.get("grant_type", "client_credentials")` returned `None` (key present with a null value), and `OAuth2AuthUtil` raised `Unsupported grant type: None` and sent an empty `Bearer`. Now defaults to `client_credentials` when `grant_type` is null/absent.
- **Angular dark mode: Processing-tab checkbox labels/outlines were invisible** — the "Skip graph" and "Enable auto change sync" checkboxes now render white labels and a visible unchecked-box outline on dark backgrounds (light mode unchanged).
- **Fresh install pulled a broken `qdrant-client 1.19.0`** (`flexible-graphrag/pyproject.toml`) — 1.19.0 moved `IDF_EMBEDDING_MODELS` out of `qdrant_client.qdrant_fastembed`, which `llama-index-vector-stores-qdrant 0.10.2` still imports from there, so a clean `uv pip install -e .` failed incremental-updates init with `ImportError: cannot import name 'IDF_EMBEDDING_MODELS'`. Pinned `qdrant-client>=1.16,<1.19` (existing envs already on 1.18 were unaffected).
- **Langflow venv failed to start after adding Nuxeo** — `flexible-graphrag` pulls `nuxeo[oauth2]`, whose GehirnInc `jwt` package owns the same top-level `jwt` module as **PyJWT** (required by Langflow) and evicts it, so `langflow run` died with `ImportError: cannot import name 'InvalidTokenError' from 'jwt'`. It's an inherent conflict (Nuxeo's OAuth2 needs the `jwt` package; Langflow needs PyJWT). Documented the per-venv fix in `docs/GETTING-STARTED/LANGFLOW-INTEGRATION.md` (install step + Troubleshooting): after installing, run `uv pip uninstall jwt` then `uv pip install --reinstall "PyJWT>=2.12.1"`. A compat shim in `sources/nuxeo.py` then lets **all 3 Nuxeo auths (basic/token/oauth2) work in Langflow flow mode** (it grafts the `jwt` symbols nuxeo imports onto PyJWT; OAuth2 uses a pre-obtained Bearer, so ingest never decodes a JWT). Alfresco + all other sources work fully in flow mode.
- **`docker-env-sample.txt` had wrong compose service names for the Fuseki/GraphDB RDF stores** — the containerized (SCENARIO B) overrides listed `fuseki:3030` and `graphdb:7200`, but the actual compose services are **`jena-fuseki`** and **`ontotext-graphdb`**, so those hostnames don't resolve from the backend/langflow containers and the RDF export silently fails (the property graph was unaffected — Neo4j's service name matched its value). Corrected to `jena-fuseki:3030` / `ontotext-graphdb:7200` (Oxigraph was already correct). Especially visible in Docker **flow mode**, where the RDF export runs inside the langflow container.
